...
$(document).ready(function () {
	...
	$('#total_found').text(
		$('#basis_sets').find('option').length + ' basis sets'
	);
	...
	function basis_sets_selection_changed() {
		...
		var elements =
			window.bs_metadata[selected]['versions'][latest_version].elements;
	...
	function update_summary(selected) {
		if (selected) {
			...
			if (notes[1]) {
				...
				$('#family_notes').attr(
					'title',
					'View notes for this basis set family'
				);
			} else {
	...
	function update_version_list(selected) {
		...
		$.each(basis_versions, function (i, version) {
			var revdesc =
				window.bs_metadata[selected]['versions'][version]['revdesc'];
	...
	function filter_basis_set_names() {
		...
		for (var i = 0; i < options_list.length; i++) {
			...
			if (
				!(
					...
					(ecp === 'ecp' &&
						basis_ecp.indexOf('scalar_ecp') === -1 &&
						basis_ecp.indexOf('spinorbit_ecp') === -1) ||
					(ecp === 'no_ecp' && basis_ecp.indexOf('scalar_ecp') > -1) ||
					basis_ecp.indexOf('spinorbit_ecp') > -1 ||
		...
		var basis_exist = options.find(
			'option[value="' + current_selected_bs + '"]'
		);
	...
	function is_basis_elements_seleced() {
		...
		if (elements.length === 0) {
			alert(
				'Please click on the element buttons to select/unselect elements you want to include for download.'
			);
			return false;
		}
